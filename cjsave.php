...
if($_GET['xz']=='1'){
	...
	$ccurl = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'];
	$tmp = stripos($ccurl, "&ur=");
	$long = substr($ccurl,$tmp+4);
	$long = str_replace('https://','http://',$long);
	$html=get_contents($long);
	
	 
	
		//20160627,agui qq383747539 修改编码乱的问题
	...
	$sql = "insert into tbl_info values (0,'".$title."','".addslashes($content)."','".$rowad['ad_img']."','".$rowad['ad_link']."','".$_COOKIE['username']."',0,0,'".date('Y-m-d')."','".$rowad['adtelnumber']."','".$ifadtop."','".$gongzhonghao."','".$ifPublicNumber."','".$rowad['erweima']."','".$ywyuedu."','".$infoid."','".$daili."','".$rowad['id']."',0,0,0,'".$titleimg."','".$adtypetop."','".$rowad['pmd_top']."','".$adtypefooter."','".$rowad['pmd_footer']."')";
