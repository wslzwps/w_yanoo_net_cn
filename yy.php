...
<div class="apply" id="apply">
	<div class="list1">
		<ul>
			<?
			if($_GET['page']){
				$count = ($_GET['page']-1)*200+1;
			}else{
				$count = 1;
			}
			$page_sql = "select * from tbl_ad order by id desc";
			qy_page($page_sql,20);
			$sql="select * from tbl_ad ORDER by id DESC";
			$_id ='&';
			...
			<li>
				<div class="list_txt">
					<a href="<?=$row['ad_link']?>"><img src="<?=$row['ad_img']?>"  style="width:calc(100% - 100px);height:60px"></a>
					<span style="float:right;font-size:12px;margin-right:10px;margin-top:-33px">
						<a href="start.php?id=<?=$row['id']?>" target="_blank">开始</a>
						<a href="stop.php?id=<?=$row['id']?>" target="_blank">停止</a>
					</span>
				</div>
			</li>
			<?
