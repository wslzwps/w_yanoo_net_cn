...
class Wechat
{
	...
	public function getRevEvent(){
		...
		if (isset($this->_receive['Ticket'])){
			$array['ticket'] = $this->_receive['Ticket'];
		}
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function createPackageXml($appid,$mch_id,$nonce_str,$body,$out_trade_no,$total_fee,$spbill_create_ip,$notify_url,$openid){
	 ...
	 * @return string
	 */
	public function createPackage($appid,$mch_id,$nonce_str,$out_trade_no,$body,$total_fee,$notify_url,$spbill_create_ip,$fee_type=1,$bank_type="WX",$input_charset="UTF-8",$time_start="",$time_expire="",$transport_fee="",$product_fee="",$goods_tag="",$attach=""){
			$arrdata = array("mch_id" => $mch_id,"bank_type" => $bank_type, "body" => $body, "partner" => $this->partnerid, "out_trade_no" => $out_trade_no, "total_fee" => $total_fee, "fee_type" => $fee_type, "notify_url" => $notify_url, "spbill_create_ip" => $spbill_create_ip, "input_charset" => $input_charset);
	...
	//退款
	public function refund($appid,$mch_id,$nonce_str,$out_trade_no,$out_refund_no,$total_fee,$refund_fee,$op_user_id)
	{
		$arrdata = array("mch_id" => $mch_id,"appid" => $appid, "nonce_str" => $nonce_str, "out_trade_no" => $out_trade_no, "out_refund_no" => $out_refund_no, "total_fee" => $total_fee, "refund_fee" => $refund_fee, "op_user_id" => $op_user_id);
		ksort($arrdata);
		$paramstring = "";
		foreach($arrdata as $key => $value)
		{
